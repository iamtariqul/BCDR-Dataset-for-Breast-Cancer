...
for i in range(5):
	plt.fill_between(desired_fps, mean_froc[i] - std_froc[i], 
					 mean_froc[i] + std_froc[i], color=colors[i], alpha=0.3)
...
plt.figure()
for i in range(25):
	plt.plot(np.linspace(0.01, 0.99, 100), iou[i], color=colors[i//5],
			 marker=markers[i%5])
